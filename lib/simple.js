...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch((_) => "https://telegra.ph/file/844cf3b7ba3deb7537b9a.jpg");
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'pwetyytataa', body: wm,mediaType: 1, previewType: 0,renderLargerThumbnail: false, sourceUrl: '', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
