...
async function toHex(text) {
	return Buffer.from(await crypto.webcrypto.subtle.digest('SHA-256', (new TextEncoder).encode(text))).toString('hex').toUpperCase() 
}
...
async function _decryptMessage(encPayload, encIv, additionalData, decryptionKey) {
	...
	const decrypted = await crypto.webcrypto.subtle.decrypt({ name: 'AES-GCM', iv: encIv, additionalData: additionalData, tagLength: 8 * tagSize_multiplier }, key, encoded);
...
async function decrypt(encKey, encPayload, encIv, pollMsgSender, pollMsgId, voteMsgSender) {
	try{
    ...
    const decryptedMessage = await _decryptMessage(encPayload, encIv, additionalData, decryptionKey)
